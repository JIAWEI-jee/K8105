...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_PREHEAT_1, DP_TYPE_BOOL},
  {DPID_SKU, DP_TYPE_RAW},
};
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
	...
	mcu_dp_bool_update ( DPID_SWITCH,get_device_state() ); //BOOL型数据上报;
	mcu_dp_value_update ( DPID_TEMP_SET,25 ); //VALUE型数据上报;
	mcu_dp_value_update ( DPID_TEMP_CURRENT,25 ); //VALUE型数据上报;
	mcu_dp_enum_update ( DPID_LEVEL,flash_info.gap ); //枚举型数据上报;
	mcu_dp_enum_update ( DPID_COUNTDOWN_SET,0 ); //枚举型数据上报;
	mcu_dp_value_update ( DPID_COUNTDOWN_LEFT,0 ); //VALUE型数据上报;
	mcu_dp_bool_update ( DPID_PREHEAT,0 ); //BOOL型数据上报;
	mcu_dp_fault_update ( DPID_FAULT,0 ); //故障型数据上报;
	mcu_dp_bool_update ( DPID_PREHEAT_1,0 ); //BOOL型数据上报;
	mcu_dp_raw_update(DPID_SKU,string); //RAW型数据上报;
}
...
static unsigned char dp_download_level_handle(const unsigned char value[], unsigned short length)
{
	...
	switch ( level )
	{
		...
		case 5:
			flash_info.gap = GAP_6;
			break;

		case 6:
			flash_info.gap = GAP_7;
			break;

		case 7:
			flash_info.gap = GAP_8;
			break;

		case 8:
			flash_info.gap = GAP_9;
			break;
...
static unsigned char dp_download_countdown_set_handle(const unsigned char value[], unsigned short length)
{
	...
	switch ( countdown_set )
	{
		...
		case 1:
			flash_info.timer = TIMER_0_5H;
			break;

		case 2:
			flash_info.timer = TIMER_1H;
			break;

		case 3:
			flash_info.timer = TIMER_2H;
			break;

		case 4:
			flash_info.timer = TIMER_4H;
			break;
...
/*****************************************************************************
函数名称 : dp_download_preheat_handle
功能描述 : 针对DPID_PREHEAT的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_preheat_handle(const unsigned char value[], unsigned short length)
{
	//示例:当前DP类型为BOOL
	unsigned char ret;
	//0:关/1:开
	...
	preheat = mcu_get_dp_download_bool ( value,length );
	if ( preheat == 0 )
	{
		//开关关
		Heat_start_std = 1;
	...
	//处理完DP数据后应有反馈
	ret = mcu_dp_bool_update ( DPID_PREHEAT,preheat );
	if ( ret == SUCCESS )
	{
		return SUCCESS;
	}
...
/*****************************************************************************
函数名称 : dp_download_preheat_1_handle
功能描述 : 针对DPID_PREHEAT_1的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_preheat_1_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为BOOL
    unsigned char ret;
    //0:关/1:开
    unsigned char preheat_1;
    
    preheat_1 = mcu_get_dp_download_bool(value,length);
    if(preheat_1 == 0) {
        //开关关
    }else {
        //开关开
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_bool_update(DPID_PREHEAT_1,preheat_1);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
/*****************************************************************************
函数名称 : dp_download_sku_handle
功能描述 : 针对DPID_SKU的处理函数
输入参数 : value:数据源数据
        : length:数据长度
返回参数 : 成功返回:SUCCESS/失败返回:ERROR
使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_sku_handle(const unsigned char value[], unsigned short length)
{
    //示例:当前DP类型为RAW
    unsigned char ret;
    /*
    ...
    */
    ...
    //处理完DP数据后应有反馈
    ret = mcu_dp_raw_update(DPID_SKU,value,length);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
...
unsigned char dp_download_handle(unsigned char dpid,const unsigned char value[], unsigned short length)
{
    ...
    ***********************************/
    ...
    switch(dpid) {
        ...
        case DPID_PREHEAT:
            //是否预热处理函数
            ret = dp_download_preheat_handle(value,length);
        break;
        case DPID_PREHEAT_1:
            //预热1处理函数
            ret = dp_download_preheat_1_handle(value,length);
        break;
        case DPID_SKU:
            //型号处理函数
            ret = dp_download_sku_handle(value,length);
        break;
